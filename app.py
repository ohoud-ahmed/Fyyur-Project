...
@app.route("/venues")
def venues():
    # List all venues inside group based state and city
    ...
    for area in listVanue:
        ...
        data.append({"city": area.city, "state": area.state, "venues": venuesData})
        ...
        response["data"].append(
            {
                "id": venue.id,
                "name": venue.name,
                "num_upcoming_shows": len(upcomingshowsSearch),
            }
        )
...
def show_venue(venue_id):
    ...
    # showInfo = Show.query.all()

    # pastShows = 0
    # upcomingShows = 0

    # for show in active_venue:
...
def artists():
    ...
    # List all artists inside group based state and city
    listArtist = Artist.query.all()
    data = []

    for artist in listArtist:
        area_venues = Artist.query.filter_by(name=artist.name).all()
        for artist in area_venues:
            data.append(
                {
                    "id": artist.id,
                    "name": artist.name,
        ...
        response["data"].append(
            {
                "id": artist.id,
                "name": artist.name,
                "num_upcoming_shows": len(upcomingSearch),
            }
        )
...
def edit_artist_submission(artist_id):
    ...
    try:
        ...
        db.session.commit()
        flash(" Artist Information edit Successfully " + artist.name)
        # TODO: on unsuccessful db insert, flash an error instead.
    except ():
        db.session.rollback()
        error = True
        flash("An error occurred. Artist " + artist.name + " could not be updates.")
...
def edit_venue_submission(venue_id):
    ...
    try:
        ...
        venue.seeking_description = request.form["seeking_description"]
        db.session.commit()
        flash(" Venue Information edit Successfully " + venue.name)
        # TODO: on unsuccessful db insert, flash an error instead.
    except ():
        db.session.rollback()
        error = True
        flash("An error occurred. Venue " + venue.name + " could not be updates.")
...
def create_artist_submission():
    ...
    try:  # on successful db insert, flash success
        artist = Artist()
        artist.name = request.form["name"]
        artist.city = request.form["city"]
        artist.state = request.form["state"]
